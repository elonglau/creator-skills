...
- `cards[].variant`：模板排版变体。MVP 必须支持 `cover`、`image_page`、`text_page`、`summary`，确保封面和内页使用不同排版规则。
- `cards[].headline`：卡片标题。
- `cards[].body`：卡片正文内容块。
...
MVP 的 `design` 风格必须覆盖四类可实际使用的排版：封面 `cover`、图文内页 `image_page`、纯文字内页 `text_page` 和总结页 `summary`。封面应保留品牌与大留白，内页应保留页码、胶囊标签、正文区、素材展示区和页脚，纯文字页与总结页不能复用图文页的图片展示框。
